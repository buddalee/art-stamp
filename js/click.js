...
function onDragStart(e) {
    ...
    if (this.data.identifier === 0 && dotNum > 0) {
        if (dotNum > 2) {
            app.stage.removeChild(app.stage.children[dotNum]);
        }
...
testGraphics.drawRect(0, 0, 800, 600);
...
function calcPhotoCenter(maxIdx) {
    ...
    const _x = touches[anotherIdx].pos.x;
    const _y = touches[anotherIdx].pos.y;
    const centerX = (x + _x) / 2;
    const centerY = (y + _y) / 2;
